...
int interface(ArrayList *decoder, char *codigo) {
  ...
  char texto_parcial[strlen(codigo)+1];
  printf("\nEscolha uma opcao:");
  printf("\n");
  ...
  switch(operacao) {
    case 1:
      mostraEstadoAtual(decoder, codigo, texto_parcial);
    break;
    case 2:
      createFreqTable(decoder, codigo);
      ArrayList decoderCopy;
      initializeArrayList(&decoderCopy);
      createFreqTable(&decoderCopy, codigo);
    ...
    case 3:
      ...
      scanf("%s", &padrao);
      getchar();
      fazCasamentoNoTextoCriptografado(codigo, padrao);
    break;
    case 4:
      printf("\nQual o padrão e a tolerância utilizados?\n> ");
      scanf("%s %d", &padrao, &tolerancia);
      fazerCasamentoNoTextoParcial(texto_parcial, padrao, tolerancia);
      mostraEstadoAtual(decoder, codigo, texto_parcial);
    ...
    case 5:
      ...
      if((checkIsValidChar(original) == TRUE) && (checkIsValidChar(mapeada) == TRUE)){
        //substitui em decode
        printf("\nVALIDOS!");
        printf("decoder->last: %d", decoder->last);
        for(int j = 0; j < decoder->last; j++) {
          printf("\nFOR!");
          DecoderStructure decoder_current =
              *(DecoderStructure*) decoder->array[j].item;
          if(decoder_current.switch_status == FALSE && decoder_current.code == original) {
            printf("CODE: %c", decoder_current.code);
            printf("CODE: %c", decoder_current.decode);
          }else{
            printf("\nELSE!");
          }
        }
...
void fazerCasamentoNoTextoParcial(char *texto_parcial, char *padrao, int erro) {
  ShiftAndAproximado(texto_parcial, strlen(texto_parcial), padrao, strlen(padrao), erro);
} 

char* criaTextoParcial(ArrayList *decoder, char* codigo) {
  char *texto_parcial = (char*) malloc(sizeof(codigo));
...
void mostraEstadoAtual(ArrayList *decoder, char* codigo, char *texto_parcial) {
  ...
  for (int i = 0; i < 26; i++) {
    for(int j = 0; j < decoder->last; j++) {
      DecoderStructure decoder_current =
          *(DecoderStructure*) decoder->array[j].item;
      if(decoder_current.switch_status == TRUE && decoder_current.decode == alfabeto[i]) {
  ...
  texto_parcial = criaTextoParcial(decoder, codigo);
...
int countCharInText(char *text) {
  ...
  for (int i = 0; i < strlen(text); i++) {
    if(checkIsValidChar(text[i]) == TRUE) qtd++;
  }
...
void fazCasamentoNoTextoCriptografado(char *codigo, char *padrao) {
  ...
  int qtd_occ = BoyerMooreHorspool(codigo, strlen(codigo), padrao, strlen(padrao), &occ);
  printf("\nOcorrencias: %d", qtd_occ);

  printf("\nOcorrencais no texto: \n");
  int occ_iterator = 0;
  for(int index_codigo = 0; index_codigo < strlen(codigo); index_codigo++) {
    int index_occ = occ_iterator < occ.last ? *(int*) occ.array[occ_iterator].item : -1;
...
void createFreqTable(ArrayList *decoder_list, char *text_encrypted) {
  ...
  for (int i = 0; i < size_text; i++) {
    ...
    // se for um char que nao seja uma letra maiuscula vamos para a proxima iteracao
    if (checkIsValidChar(current_encrypted_char) == FALSE) continue;

    int index_char_decoder = checkForCharInDecoderList(decoder_list, current_encrypted_char);
  ...
  for(int i = 0; i < decoder_list->last; i++) {
    DecoderStructure decode_item = *(DecoderStructure*) decoder_list->array[i].item;
    sum_occurrences += decode_item.occurrence;
  }
  
  for(int i = 0; i < decoder_list->last; i++) {
    DecoderStructure *decode_item = (DecoderStructure*) decoder_list->array[i].item;
    decode_item->freq = ((float)decode_item->occurrence * 100.00) / sum_occurrences;
  }
}
...
void imprimeTabelaFreq(ArrayList *decoder_list) {
  ...
  for(int i = 0; i < decoder_list->last; i++) {
    ...
    printf("\n%-1c      |        %-4d|  %-2.2lf%c", decode_item.code, decode_item.occurrence, decode_item.freq, '%');
  }
}
...
void createFreqTablePortBR(ArrayList *list) {
  ...
  float freqArr[26] = {14.63, 1.04, 3.88, 4.99, 12.57, 1.02, 1.30, 1.28, 6.18, 0.40, 0.02, 2.78, 4.74, 5.05, 10.73, 2.52, 1.20, 6.53, 7.81, 4.34, 4.63, 1.67, 0.01, 0.21, 0.01, 0.47}; 
...
void imprimeFreqTablePortBR(ArrayList list) {
  ...
  for(int i = 0; i < list.last; i++) {
    ...
    printf("\n%-1c      |  %-2.2lf%c", decode_item.letraAlfabeto, decode_item.freq, '%');
  }
}
